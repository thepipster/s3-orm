...
*/
    ...
    test('ordered set add/remove', async () => {
        ...
        // check it got added
        items = await s3Client.zSetMembers(setName);
    ...
    test('ordered set query', async () => {
        ...
        let items = await s3Client.zSetMembers(setName, true);
        expect(items.length).toEqual(words.length);

        let rangeItems = null;

        // Test gte/lte
        rangeItems = await s3Client.zRange(setName, {'$gte':3, '$lte':6, score: true});          
        ...
        // Negatives
        rangeItems = await s3Client.zRange(setName, {$gte:-4, $lt:0, score: true}); 
        expect(rangeItems.length).toEqual(2);        
        expect(rangeItems[0].score).toEqual(-1);
        expect(rangeItems[1].score).toEqual(-4);

        // Zeros
        rangeItems = await s3Client.zRange(setName, {$gte:0, $lt:3, score: true});     
