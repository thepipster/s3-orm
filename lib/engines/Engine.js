...
class Engine extends BaseS3Engine {
   ...
   // }
    ...
    async zRange(setName, opts){
        ...
        function isNull(val){
            return _.isNull(val) || _.isUndefined(val) || val != 0;
        }
        ...
        for (let i=0; i<res.length; i+=1){
            ...
            Logger.debug(`zRange() 
                score = ${item.score}, 
                opts.$lt = ${opts.$lt}, ${isNull(opts.$lt)},
                opts.$lte = ${opts.$lte}, ${isNull(opts.$lte)},
                opts.$gt = ${opts.$gt}, ${isNull(opts.$gt)},
                opts.$gte =${opts.$gte},  ${isNull(opts.$gte)},
                lowerFlag = ${lowerFlag}, 
                upperFlag = ${upperFlag}`);
                */
