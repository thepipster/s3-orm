...
const DataTypes = {
    ...
    Id: {
        ...
        encode(val){ return DataTypes._cleanString(val+'') }        
    },
    Uuid: {
        name: 'uuid',
        isNumeric: false,
    ...
    Array: {
        name: 'array',
        isNumeric: false,
        mock(){ return chance.n(chance.word, 5)},
        parse(val){ 
            if (val && typeof val == 'string'){
                try {
                    return JSON.parse(val)
        ...
        encode(val){ return JSON.stringify(val) }
    },
    Date: {
        name: 'date',
        isNumeric: false,
    ...
    /**
     * Deal with null or undefined values that got encoded
     * @param {*} str 
     * @returns 
     */
    _cleanString(str){
