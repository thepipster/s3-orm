...
/*
    ...
    */
    ...
    async removeIndexForField(key, val){
        ...
        const encodedVal = fieldDef.type.encode(val);
        ...
        try {
            ...
            if (fieldDef.unique) {
                await this.removeUnique(key, encodedVal);
            }
                            
            if (fieldDef.type.isNumeric) {
                await this.removeNumeric(key, encodedVal);
            }
            else {
                await this.remove(key, encodedVal);                
            }
    ...
    async setIndexForField(key, val, oldVal){
        ...
        const encodedVal = fieldDef.type.encode(val);
        //encodedOldVal = fieldDef.type.encode(oldVal);
        ...
        */
        ...
        if (fieldDef.unique) {
            await this.addUnique(key, encodedVal);
        }                

        if (fieldDef.type.isNumeric && !isNull) {
            await this.addNumeric(key, encodedVal);
        }
        else {
            await this.add(key, encodedVal);
        }
