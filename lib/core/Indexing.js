...
class Indexing {
    ...
    async remove(fieldName, val){
        ...
        //let key = `${Indexing.getIndexName(this.modelName, fieldName)}/${val}###${this.id}`;
        this.s3.setRemove(`${Indexing.getIndexName(this.modelName, fieldName)}/${val}###${this.id}`);
    ...
    static async getIdsForVal(fieldName, val){
        // Stuff the id into the index as a meta value
        //await this.s3.setAdd(Indexing.getIndexName(this.modelName, fieldName), val, this.id);
        let idList = await this.s3.setMembers(`${Indexing.getIndexName(this.modelName, fieldName)}/${val}###`);
        return idList;
        //await this.s3.setAdd(`${Indexing.getIndexName(this.modelName, fieldName)}/${this.id}###${val}`);            
    }
