...
class BaseModel {
    ...
    async save() {
        ...
        if (this.id){
            ...
            this.id = maxId + 1;
            Logger.debug(`[${chalk.green(modelName)}] setting max id to ${this.id}`);
            await indx.setMaxId(this.id);
            Logger.debug(`[${chalk.green(modelName)}] maxId (after) = ${await indx.getMaxId()}`);
        }
    ...
    static async getIds(query, options) {
        ...
        // Support paging
        if (options.offset){
            inter = _.slice(inter, options.offset);
        }
        if (options.limit){
            inter = _.slice(inter, 0, options.limit);
        }
