...
class BaseModel {
    ...
    async save() {
        ...
        await Promise.map(keys, async (key)=>{
            ...
            try {
                if (isDirty){
                    indx.setIndexForField(definition, val, oldVal);
                }            
            } 
            catch (err) {
    ...
    static async loadFromId(id) {
        try {
            ...
            for (let key in model) {
                ...
                if (key in data) {
                    doc[key] = (model[key].type) ? model[key].type._decode(data[key]) : model[key]._decode(data[key]);
                }
            }
    ...
    // ///////////////////////////////////////////////////////////////////////////////////////
/*
    static async _basicIndexSearch(qry) {
    ...
    */
    ...
    static async getIds(query = {}) {
        ...
        const basicIndexes = structuredSearches.filter((search) => search.type === "set");
        const numericIndexes = structuredSearches.filter((search) => search.type === "zset");

        Logger.debug('basicIndexes = ', basicIndexes)
        Logger.debug('numericIndexes = ', numericIndexes)
        Logger.info('structuredSearches ', structuredSearches);

        // Get results from basic index searches
        var results = [];
        await Promise.map(basicIndexes, async (qry) => {
            results.push(await indx.search(qry.key, qry.value));
        });

        //Logger.info(results);
        
        // Get results from numeric index searches
        await Promise.map(numericIndexes, async (qry) => {

            let res = await indx.searchNumeric(qry.key, qry.value);
            //results.push(await indx.search(qry.key, qry.value));
        });
    ...
    static generateMock() {
        ...
        //return BaseModelHelper.generateMock(this._schema());
    }
